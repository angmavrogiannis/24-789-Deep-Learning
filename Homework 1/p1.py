...
# Needs mnist.train and mnist.test files to run
...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        norm_sum = np.sum(exp_logits, axis=1)
        self.softmax = exp_logits / norm_sum[:, None]
        loss = - np.sum(np.sum(self.labels * np.log(self.softmax), axis=1))
        return loss
...
class MLP(object):
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            a = np.dot(self.inputs[i], self.W[i]) + self.b[i]
            self.activation_inputs[i] = a
            self.inputs[i+1] = self.activations[i](a)
    ...
    def backward(self, labels):
        if self.train_mode:
            ...
            self.zero_grads()
            self.loss = self.criterion(self.inputs[-1], labels)
            deriv = self.criterion.derivative() / self.inputs[0].shape[0]

            for i in reversed(range(self.nlayers)):
                deriv = self.activations[i].derivative() * deriv
                self.dW[i] = np.dot(self.inputs[i].T, deriv)
                self.db[i] = np.dot(deriv.T, np.ones((self.inputs[i].shape[0], 1))).reshape(-1)
                deriv = np.dot(deriv, self.W[i].T)
    ...
    def get_error(self, labels):
        ...
        num_mistakes = 0
        for y, y_hat in zip(labels, self.criterion.softmax):
